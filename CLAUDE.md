...
- **DateField (cumpleaños / campos `date`):** no usar `input[type=date]` (el nativo no abre bien dentro del fancy). El popup flota en `document.body` con host `position:fixed; inset:0` encima del overlay. `.gafa-datepicker` del calendario de clases trae `top: calc(100% + 8px)` y `transform`; `.gafa-datepicker--floating` **tiene que resetearlos**. Si no, en móvil el cumpleaños abre hacia arriba y el widget queda fuera de pantalla (Hybrix 2026-09-17). Preview: `/preview.html?guest=1` → Crear cuenta.
...
- **THEME.headerControls (2026-09-04):** chrome oficial de `Entrar` / `Mi cuenta` (`.gafa-header-account`). Sin el bloque, el botón queda como hoy. El carrito circular no se toca. Vars `--gafa-header-account-*`. Guía: `docs/v2-agente.md` §5.1. No CSS externo ni observers.
...
- **Concierge (en el embed desde 2026-09-04):** apagado salvo `CONCIERGE`, venga del Hub o del HTML. Encendido flota en todo el sitio: sin nodo el bootstrap cuelga el suyo del `body`; el nodo solo manda **dónde** va y `data-gf-concierge="off"` excluye una página. Nodo sin config **no tumba** el resto del SDK (`console.warn`). WhatsApp opcional. `products: []` + `live: true` = catálogo de esa `COMPANY_ID`. ☀/☾ (si `allowUserColorScheme`) cambia toda la página. Hard refresh; **no Republish**.
- **Concierge CSS (2026-09-04):** vive dentro de `.gafa-sdk`, así que la muralla anti-Elementor aplasta `button`/`input` con `--gafa-control-*` / `--gafa-field-*`. Hay que setear esos tokens (`.gafa-concierge-chip`, `.gafa-concierge-input`) o las pastillas quedan texto plano enorme y el input sale negro del THEME dark de Fitspin. Superficies arrancan en `CONCIERGE.theme.mode` (Fitspin = light), no en el THEME del host. Toggle ☀/☾ en el header cambia solo el chat. El input usa `--concierge-field-bg`, no `--gafa-color-input-background`.
...
- **Auth (login/registro/password) — RESUELTO (2026-08-05):** `httpGafaClient.ts` implementa `login/register/requestPasswordReset/resetPassword/getProfile/logout` directo, **ya no delega a `window.GafaFitSDK` para nada de esto** (a diferencia del gap original). Contrato exacto (no adivinado, sacado del código fuente real del SDK legacy que vive en `gafa.fit/resources/assets/js/sdk/{GafaFitSDK,GafaFitRequests}.js`, `1413`+`153` líneas):
  ...
  - **Captcha default (2026-09-17):** el toast `Falta configurar el captcha (captchaPublicKey)…` **no debe salir**. `resolveCaptchaKeys()` exige el par completo (público + secret) o cae al par compartido de Buq. Una sola llave (típico: Hub guarda `CAPTCHA_PUBLIC_KEY` y el secret está prohibido en el Hub) **no** pisa el default — si no, Google valida el token con otro secret y gafa.fit responde 422. `""` / whitespace tampoco pisan. `createCaptchaProvider()` siempre devuelve proveedor. `AuthWidget` usa `captcha ?? createCaptchaProvider()`. `openCheckout` tiene que pasar `captcha` al `CheckoutModal` (si no, Crear cuenta en pagar fallaba aunque el SDK ya tenía el default). Preview `/preview.html?guest=1` monta un captcha stub: no carga Google. Socio con par propio en el HTML: se usa tal cual.
...
- **reCAPTCHA:** `CaptchaProvider` con v3 default y Turnstile detrás del mismo contrato. El par compartido de Buq es el default del SDK; el Hub no guarda el secret. Gotcha: pública suelta del Hub ≠ par válido. Ver nota *Captcha default* en Auth.
