...
- **Imágenes de marca (fotos de coach) — RESUELTO en el SDK, FALTA un switch en Cloudflare (2026-08-11):** la API devuelve la imagen **original tal cual la subió la marca**, y las cinco "variantes" que expone (`picture_web`, `picture_web_list`, `picture_web_over`, `picture_movil`, `picture_movil_list`) son **copias byte a byte** — se comparó el `Content-MD5` de los 103 coaches con foto de Fitspin (company 80) y coincide en las cinco. Ninguna está redimensionada, así que gafa.fit guarda 5 copias del mismo archivo por coach.
  ...
  - **Lo que falta y NO se puede hacer desde este repo:** en el dashboard de Cloudflare, activar Images > Transformations en la zona y agregar `*.blob.core.windows.net` en Sources (por default solo se aceptan imágenes de la propia zona). Hoy `/cdn-cgi/image/...` responde 404 en buq.partners porque la función no está activada. Mientras tanto el SDK lo detecta con el primer error, lo apaga por toda la sesión (`sessionStorage['gafa-sdk-image-transforms']`) y no baja ningún original: las fotos de coach simplemente no se pintan (`allowUnoptimizedOriginals: true` revierte eso). Costo Cloudflare: 5,000 transformaciones únicas/mes gratis, después $0.50 por 1,000.
