...
- **Tres backends de Buq (ago-2026):** production `buq.partners` (default, lanzamiento), staging `buq.com.mx` (Stripe nuevo + Laravel listo para subir), development `buq.technology`. Se cambian con `BUQ_ENV` / `environment` / `GAFA_FIT_URL` / `?buq-env=staging`. GafaPayFront se deduce del entorno; se pisa con `GAFAPAY_FRONT_URL`. Tokens de staging/dev van a otra key de localStorage para no desloguear producción en el mismo dominio. Embed WordPress: `npm run build:embed` → `docs/v2-embed/gafa-embed.js`.
