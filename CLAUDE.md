...
- **Integración (2026-09-03):** `v2/main` ya trae `cdn-live` (checkout + hold de PayPal) y el Dashboard Hub. **Concierge no se mezcla** (`cursor/sdk-public-facade-v2-6ed8` sigue en su rama). Features nuevos se ramifican desde `v2/main`. El embed público sigue en `cdn-live`; el Worker del Hub se despliega aparte (`packages/sdk-hub`, no Republish).
- **Overlays (login/reserva/checkout/cuenta):** van por portal a `document.body` (`SdkBodyOverlay`). Si se quedan dentro del calendario, Elementor/`transform` los centran solo en esa sección. El logo del estudio es `THEME.logoUrl` (light) o `THEME.logoUrlDark` (dark; si no hay, cae a `logoUrl`) o el `pic` público de la marca — wordmark compacto (`img.gafa-studio-logo`, tope 40×120, `width:auto !important`) porque Elementor pone `img{width:100%}` y la aislación default (`max-width:none`) lo infla al PNG intrínseco. Solo Fitspin elige dark/light hoy. En checkout sin sesión el form de login llena la columna izquierda; el carrito se queda. **Toasts (prueba 2026-09-04):** opción 1, arriba a la derecha (`.gafa-toast-stack`, z 2147483003). Solo login/registro — el banner `.gafa-sdk-state--error` de esos forms ya no empuja el layout; los bordes rojos (`data-invalid`) se quedan. Recuperar contraseña y el resto de alertas del SDK siguen inline. Stripe en dark: GafaPay hardcodea `#303238` en el Card Element y a veces lo vuelve a poner con `update()`; el SDK parchea `window.Stripe` (`stripeCardStyle.ts`: `create`, `update` y `appearance` night). El iframe no se pinta con CSS. **En `cdn-live` desde 2026-09-04** (`gafa-sdk.bundle.20260904T004954Z.js`; toasts login/registro + Stripe `update()`/`appearance`). En checkout auth el form llena la columna con 1.5rem extra por lado.
- **Theme del host:** la muralla anti-Elementor impide que el CSS del sitio pinte `.gafa-sdk`. Fitspin manda `THEME.colorScheme:"light"` fijo y pinta `--sdk-*` en `html.fitspin-dark` (overlay `fitspin-gftheme.css`, clases legacy `GFSDK-*`). El SDK V2 observa la clase / `fitspin-theme` / `--sdk-*` y además resuelve `--gafa-color-*` como `var(--sdk-text-color, …)` para que el toggle de la página pinte el calendario sin JS. No tocar el overlay de Buq-Webs ni Republish.
