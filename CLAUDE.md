...
- **Imágenes de marca (fotos de coach) — RESUELTO end to end (2026-08-11):** la API devuelve la imagen **original tal cual la subió la marca**, y las cinco "variantes" que expone (`picture_web`, `picture_web_list`, `picture_web_over`, `picture_movil`, `picture_movil_list`) son **copias byte a byte** — se comparó el `Content-MD5` de los 103 coaches con foto de Fitspin (company 80) y coincide en las cinco. Ninguna está redimensionada, así que gafa.fit guarda 5 copias del mismo archivo por coach.
  - **Tamaño del problema (medido contra producción, no estimado):** 273 MB en fotos de coach solo en Fitspin, 13 de más de 10 MB, la peor de 16.3 MB / 4000x6000 px. Con el widget v2 real, la vista de semana descargaba **109.7 MB** en 19 imágenes distintas para pintarlas en círculos de 36 px; en un móvil a 4 Mbps, a los 116 s solo habían terminado 8 de 16 fotos y la peor tardó 82.6 s.
  - **Solución en el SDK:** `src/sdk/images/` pide miniaturas a las Transformations de Cloudflare de la misma zona de `apiBaseUrl` (buq.partners ya es zona de Cloudflare: responde `server: cloudflare`). Formato `https://<zona>/cdn-cgi/image/<params>/<url original>` con la original concatenada **sin url-encodear**. Medido con la misma semana: 109.7 MB → **15.9 KB**, mismos 95 avatares en pantalla.
  - **Configuración de Cloudflare — YA HECHA en la zona buq.partners (2026-08-11):** Images > Transformations activado, y en Sources la opción "Orígenes especificados" con `buqstorage.blob.core.windows.net` (ese es el único host de imágenes que usan las compañías; se verificó con Fitspin y con Buq). Verificado en vivo: la foto de 15.2 MB sale como AVIF de 937 bytes, y el edge la sirve en 37 ms con `cf-cache-status: HIT`. **Si algún día `/cdn-cgi/image/...` empieza a responder `403`, el problema es Sources** (origen no permitido), y si responde `404` es que se desactivaron las transformaciones de la zona. Costo: 5,000 transformaciones únicas/mes gratis, después $0.50 por 1,000.
  - **Si la zona se cae o se apaga, el SDK no se rompe:** detecta el primer error, apaga las transformaciones por toda la sesión (`sessionStorage['gafa-sdk-image-transforms']`) y no baja ningún original — las fotos de coach simplemente no se pintan (`allowUnoptimizedOriginals: true` revierte eso y vuelve a pintar los originales pesados).
  - **Detalle no obvio del storage:** el blob **no manda `Cache-Control`** (solo ETag/Last-Modified, o sea caché heurística), va por HTTP/1.1 y **no tiene CORS**, así que cualquier idea de redimensionar en el cliente (canvas, Service Worker) está bloqueada de origen. El arreglo de fondo sigue siendo que gafa.fit genere las variantes de verdad al subir la imagen.
...
- **Sitio de prueba local conectado a producción — LISTO:** `packages/react-sdk/live.html` + `src/main.live.tsx`, credenciales en `packages/react-sdk/.env.local` (gitignored, nunca commitear). Compañía de prueba real: `COMPANY_ID=143` (brand "Buq", slug `buq-1`, location "CDMX" id `235`), `GAFA_FIT_URL=https://buq.partners`. Pagos de esa marca en Stripe test mode. Correr con `npm run dev -- --port <libre>` y abrir `/live.html`. `VITE_GAFA_IMAGES_ZONE` (opcional) apunta las miniaturas a otra zona; sin ella se usa el host de la API.
