...
- **Estado real por widget (verificado corriendo el preview, no solo leyendo código):**
  ...
  | `AuthWidget.tsx` | 🚧 Cascarón | Tabs Login/Registro/Password cambian bien de vista y campos, pero sin submit, sin validación, sin reCAPTCHA. Es el punto natural para interceptar `NotAuthenticatedError` (ver abajo) y mostrar login inline en vez de solo un error. |
...
- **Orden de construcción acordado:** 1) ~~Calendario~~ → 2) ~~Fancy/checkout~~ (ambos con datos/flujo real) → 3) Login/Registro (+ reCAPTCHA) → 4) Perfil → 5) Paquetes/membresías (lo que menos se usa hoy; construir y solo registrar su ID).
