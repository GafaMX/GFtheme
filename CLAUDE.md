...
- **Integración (2026-09-04):** `v2/main` ya trae `cdn-live` (checkout + hold de PayPal), el Dashboard Hub y **Concierge nativo** (opt-in: nodo `data-gf-theme="concierge"` + `CONCIERGE` en options; WhatsApp opcional; `catalog.live` + `products: []` hidrata toda la compañía). Guía: `docs/v2-agente.md` §11. Features nuevos se ramifican desde `v2/main`. El embed público sigue en `cdn-live`; el Worker del Hub se despliega aparte (`packages/sdk-hub`, no Republish).
- **Remote config (2026-09-08):** el Hub guarda un **partial** por compañía (no el objeto resuelto). Merge: defaults SDK → Hub → `[data-gf-options]` → query (`?buq-env`, `?hub-url`). HTML mínimo: `COMPANY_ID` + `API_CLIENT` + `API_SECRET`. El secret **nunca** va al Hub. `CONCIERGE: true | {} | partial` usa `createLiveConciergeConfig()`; el nodo HTML sigue siendo opt-in por página. Contrato: `docs/v2-hub/remote-config.md`. Catálogo: `docs/v2-options.md`. Admin primero en `hub.buq.partners` (mismo PUT que usará Buq-Webs después). No Republish.
- **Overlays (login/reserva/checkout/cuenta):** van por portal a `document.body` (`SdkBodyOverlay`). Si se quedan dentro del calendario, Elementor/`transform` los centran solo en esa sección. El logo del estudio es `THEME.logoUrl` o el `pic` público de la marca. Stripe en dark: GafaPay hardcodea `#303238` en el Card Element; el SDK parchea `window.Stripe` (`stripeCardStyle.ts`) porque el iframe no se pinta con CSS. **En `cdn-live` desde 2026-09-03** (`gafa-sdk.bundle.20260903T183329Z.js`).
