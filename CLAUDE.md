...
- **CDN (jsDelivr) / Buq-Webs:** la app Replit sirve **todas** las marcas (`web.buq.mx/<marca>`). Un secreto `VITE_GAFA_SDK_V2_URL` → `https://cdn.jsdelivr.net/gh/GafaMX/GFtheme@cdn-live/docs/v2-sdk/gafa-sdk.js`. Fitspin es el piloto de QA, no el único destino. WordPress (p.ej. masvoltio.com) a veces pinea un SHA: hay que cambiar el `src` a `@cdn-live`. `@v2/main` 404; `@v2` y tags `rc.*` se congelan. Cada publish: `npm run publish:embed` → commit `docs/v2-sdk/` → `git push origin HEAD:refs/heads/cdn-live` → purge. **No Republish** (cae toda la app). Receta: `docs/v2-lanzamiento.md`.
...
- **No pulses Republish ni Stop/Run en Buq-Webs para publicar V2.** El artefacto es `docs/v2-sdk/gafa-sdk.js` (`npm run publish:embed`) empujado a la rama `cdn-live`. Copiar `src/` a Replit o Republish reinicia **todas** las marcas de la app, no un sitio. Receta: `docs/v2-lanzamiento.md`.
