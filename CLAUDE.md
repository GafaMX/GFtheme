...
- **Overlays (login/reserva/checkout/cuenta):** van por portal a `document.body` (`SdkBodyOverlay`). Si se quedan dentro del calendario, Elementor/`transform` los centran solo en esa sección. El logo del estudio es `THEME.logoUrl` o el `pic` público de la marca — wordmark compacto (`img.gafa-studio-logo`, tope 40×120, `width:auto !important`) porque Elementor pone `img{width:100%}` y la aislación default (`max-width:none`) lo infla al PNG intrínseco. Stripe en dark: GafaPay hardcodea `#303238` en el Card Element; el SDK parchea `window.Stripe` (`stripeCardStyle.ts`) porque el iframe no se pinta con CSS. **En `cdn-live` desde 2026-09-03** (`gafa-sdk.bundle.20260903T193356Z.js`; logo tope 40×120).
