...
- **Concierge (en el embed desde 2026-09-04):** apagado salvo nodo HTML + `CONCIERGE`. El nodo va **solo en la página pedida**, nunca en el layout/header global. Nodo vacío **no tumba** el resto del SDK (`console.warn`). WhatsApp opcional. `products: []` + `live: true` = catálogo de esa `COMPANY_ID`. Sin nodo/config los sitios actuales no ven barra. ☀/☾ (si `allowUserColorScheme`) cambia toda la página. Hard refresh; **no Republish**.
...
- **Remote config + admin SDK (plan 2026-09-08, no implementado):** todo en este repo. Fuente de verdad = Zod del SDK; Hub guarda un partial. `CONCIERGE: true` / `{}` hereda `COMPANY_ID` + `THEME` vía `createLiveConciergeConfig()` (existe, no está cableado). Nodo Concierge sigue por página. Admin primero en Hub; Buq-Webs después, mismo contrato. Handoff: `docs/v2-hub/remote-config.md`. Orden: fase 1 (defaults Concierge) → catálogo `docs/v2-options.md` → schema/D1/fetch → admin Hub. `API_SECRET` nunca al Hub. No Republish.
