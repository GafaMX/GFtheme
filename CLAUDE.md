...
- **Deploy del Hub (2026-09-09):** `hub.buq.partners` se sube con `cd packages/sdk-hub && npm run deploy:production` (cuenta Cloudflare BUQ, `i@gafa.mx`). **No** es `cdn-live`. Si `wrangler whoami` falla: `npx wrangler login --device --browser=false` → https://dash.cloudflare.com/oauth2/device/verify → pegar el código → confirmar. Luego el deploy. Detalle: `packages/sdk-hub/README.md` § Deploy. No inventes un token si el device login basta.
- **Aparcado:** Concierge conversacional (reservar/comprar en el chat). Plan en [`docs/v2-parked.md`](docs/v2-parked.md). No implementarlo hasta que lo pidan.
