...
- **Theme del host:** la muralla anti-Elementor impide que `html.fitspin-dark` pinte el SDK. El SDK observa la clase/`data-theme` de `html`/`body` y cambia su propia paleta. Fitspin manda `THEME.colorScheme:"light"` fijo; la clase de la página le gana.
