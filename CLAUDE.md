...
- **Estado real por widget (verificado corriendo el preview, no solo leyendo código):**
  ...
  | `ProfileWidget.tsx` | 🚧 Cascarón | Estático. Bug de CSS conocido: "creditos" y "reservas" se renderizan sin espacio entre sí (falta gap en `widgets.css`). Ya puede usar `client.getProfile()` (real, `GET /api/me`) en cuanto se construya. |
