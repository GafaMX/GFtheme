...
class StaffList extends React.Component {
    ...
    render() {
        ...
        if (filter_brand && filter_brand != 'TODOS') {
            list = list.filter(function (person) {
                return person.brand.name.toUpperCase() === filter_brand
            });
        }
                    ...
                    <div className={filterClass}>
                        {jobs.length > 1
                            ? <div className={filterClass + '__item is-job-filter'}>
                                <select className={formClass + '__select' + ' is-service-filter'} onChange={this.change}
                                        value={filter_job} data-name={"filter_job"}>
