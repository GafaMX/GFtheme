...
class ServiceList extends React.Component {
    ...
    setInitialValues() {
        ...
        if (weAreHome === true) {
            services = services.filter(function (service) {
                return (service.hide_in_home === false || service.hide_in_home === 0)
            });
        }
    ...
    render() {
        ...
        if (filter_category && filter_category != 'TODOS') {
            list = list.filter(function (service) {
                if (service.category) {
                    return service.category && service.category.toUpperCase() === filter_category
                }
            });
        }
        ...
        if (filter_brand && filter_brand != 'TODOS') {
            list = list.filter(function (service) {
                return service.brand.name.toUpperCase() === filter_brand
            });
        }
                    ...
                    <div className={filterClass}>
                        {categories.length > 1
                            ? <div className={filterClass + '__item is-job-filter'}>
                                <select className={formClass + '__select' + ' is-category-filter'}
                                        onChange={this.change} value={filter_category} data-name={"filter_category"}>
                        ...
                        {brands.length > 1
                            ? <div className={filterClass + '__item is-brand-filter'}>
                                <select className={formClass + '__select' + ' is-brand-filter'} onChange={this.change}
                                        value={filter_brand} data-name={"filter_brand"}>
                                    <option value={'TODOS'}>Todos</option>
                                    {brands.map(brand => {
